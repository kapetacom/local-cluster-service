...
class ProgressListener {
    ...
    run(command:string, directory?:string):Promise<{exit:number,signal:NodeJS.Signals|null}> {
        ...
        return new Promise((resolve, reject) => {
            ...
            child.on('exit', (exit, signal) => {
                if (exit !== 0) {
                    this.socketManager.emit(`install`, 'install:log', {type: 'info', message: `"${command}" failed: "${exit}"`});
                    reject(new Error(`Command "${command}" exited with code ${exit}`));
                } else {
                    this.socketManager.emit(`install`, 'install:log', {type: 'info', message: `Command OK: "${command}"`});
                    resolve({exit, signal});
                }
            });

            child.on('error', (err) => {
                this.socketManager.emit(`install`, 'install:log', {type: 'info', message: `"${command}" failed: "${err.message}"`});
                reject(err);
            });
        });
    }
    ...
    async progress(label:string, callback:() => void|Promise<void>) {
        ...
        try {
            ...
            this.socketManager.emit(`install`, 'install:log', {type: 'info', message: `${label}: failed. ${e.message}`});
            throw e;
        }
    }
